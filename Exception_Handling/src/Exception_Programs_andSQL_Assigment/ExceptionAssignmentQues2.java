...
public class ExceptionAssignmentQues2 {
	...
	public static void main(String args[]){
		...
		for (int i=0; i <= result.length-1;i++)
			 a += " "+ result[i];
		System.out.println("Result with Interger division and No Errors: "+a);
		for (int i=0; i <= result1.length-1;i++)
			b += " "+result1[i];
		System.out.println("Result after Integer division Error: "+b);
		for (int i=0; i <= result2.length-1;i++)
			 c += " "+ result2[i];
		System.out.println("Result after Zero division Error: "+c);
